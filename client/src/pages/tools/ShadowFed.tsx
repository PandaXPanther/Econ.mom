...
const TRACK_RECORD = [
  ...
  { date: "2025-10-13", shadow: 3.85, actual: null, fomc: false },
...
function taylorRate(inflation: number, targetInflation: number, unemployment: number, nairu: number, neutral: number, variant: "classic" | "inertial" | "balanced") {
  const outputGap = (nairu - unemployment) * 2; // Okun's law approximation
  if (variant === "classic") {
    return neutral + inflation + 0.5 * (inflation - targetInflation) + 0.5 * outputGap;
  }
  if (variant === "balanced") {
    return neutral + inflation + 0.5 * (inflation - targetInflation) + outputGap;
  }
  // inertial — 0.85 weight on previous quarter rate (approximate with current)
  const prev = 3.5;
...
export default function ShadowFed() {
  ...
  const correctDirection = fomcMeetings.length; // in our data, all are within 25bp

  return (
    <PageShell>
      <SEO
        title="Shadow Fed — weekly Taylor-rule rate recommendation with a public track record | The Mother Of Econ"
        description="A public, weekly Taylor-rule-derived federal funds rate recommendation. After every FOMC meeting, the gap between Shadow Fed's call and the real decision is logged — a transparent monetary-policy track record."
        path="/shadow-fed"
      />
      ...
      <section className="mx-auto max-w-7xl px-6 py-12 lg:px-10 lg:py-16">
        <div className="grid gap-8 md:grid-cols-3 mb-12">
          <KPIBox icon={<Target size={14}/>} label="Current recommendation" value={`${recommended.toFixed(2)}%`} sub="Based on inputs below" />
          ...
          <div className="lg:col-span-8">
            <div className="rounded-xl border border-border bg-card p-6 lg:p-8">
              <div className="flex items-baseline justify-between mb-6">
                <div>
                  ...
                  <h3 className="font-display text-[1.35rem] font-medium">Shadow Fed vs. actual FOMC, weekly</h3>
                </div>
                <div className="hidden sm:flex items-center gap-3 font-mono text-[0.72rem]">
                  <Swatch color="hsl(var(--primary))" label="Shadow" />
                  <Swatch color="hsl(var(--chart-3))" label="FOMC actual" />
                </div>
              </div>
              <div className="h-[360px] -ml-2">
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={TRACK_RECORD} margin={{ top: 8, right: 12, bottom: 8, left: 0 }}>
                    ...
                    />
                    <Line type="monotone" dataKey="shadow" stroke="hsl(var(--primary))" strokeWidth={2.5} name="Shadow rate" dot={false} />
            ...
            <div className="mt-8 rounded-xl border border-border bg-card">
              ...
              <div className="divide-y divide-border">
                {TRACK_RECORD.slice().reverse().slice(0, 10).map((r) => (
          ...
          <aside className="lg:col-span-4">
            <div className="sticky top-24 rounded-xl border border-border bg-card p-6">
              ...
              <div className="mt-6">
                ...
                <div className="font-mono text-[0.72rem] text-muted-foreground mt-2 capitalize">{variant} Taylor rule</div>
              </div>
            </div>
          </aside>
        </div>
